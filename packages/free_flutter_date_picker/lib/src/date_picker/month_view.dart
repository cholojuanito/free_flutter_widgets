...
class MonthView extends StatefulWidget {
  ...
  /// Holds the blackout dates of the [DatePicker].
  final List<dynamic>? blackoutDates;

  /// Holds the special dates of the [DatePicker].
  final List<dynamic>? specialDates;

  /// Holds the list of week day index of the [DatePicker].
  ...
  /// Defines the selection mode of the [DatePicker].
  ...
  /// Specifies the pickerType for [DatePicker].
  final bool isHijri;
...
abstract class _IMonthView extends RenderBox
  ...
  set blackoutDates(List<dynamic>? value) {
    if (DatePickerHelper.isDateCollectionEquals(_blackoutDates, value)) {
      return;
    }
  ...
  set specialDates(List<dynamic>? value) {
    if (DatePickerHelper.isDateCollectionEquals(_specialDates, value)) {
      return;
    }
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    for (int j = 0; j < viewCount; j++) {
      final int currentViewIndex =
          _isRtl ? DatePickerHelper.getRtlIndex(viewCount, j) : j;
      ...
      for (int i = 0; i < datesCount; i++) {
        ...
        if (!DatePickerHelper.isDateAsCurrentMonthDate(middleDate, _rowCount,
            _showLeadingAndTailingDates, currentDate, _isHijri)) {
          ...
          continue;
        } else if (DatePickerHelper.isDateWithinVisibleDates(
          ...
          top = leftAndTopValue['top']!;
          continue;
        } else if (!DatePickerHelper.isEnabledDate(
  ...
  String _getSemanticMonthLabel(dynamic date) {
    if (_isHijri) {
      return DateFormat('EEE').format(date.toDateTime()).toString() +
          ...
          date.day.toString() +
          '/' +
          DatePickerHelper.getHijriMonthText(date, localizations, 'MMMM') +
          '/' +
          date.year.toString();
    } else {
...
class _MonthViewSingleSelectionRenderObject extends _IMonthView {
  ...
  List<int> getSelectedIndexValues(int viewStartIndex, int viewEndIndex) {
    ...
    if (selectedDate != null) {
      if (selectedDate.isDateWithInDateRange(
          visibleDates[viewStartIndex], visibleDates[viewEndIndex])) {
  ...
  @override
  void updateSelection(PickerStateArgs details) {
    if (selectedDate.isSameDate(details.selectedDate)) {
      return;
    }
...
class _MonthViewMultiSelectionRenderObject extends _IMonthView {
  ...
  void updateSelection(PickerStateArgs details) {
    ...
    selectedDates = DatePickerHelper.cloneList(details.selectedDates);
    selectionNotifier.value = !selectionNotifier.value;
  }
}
...
class _MonthViewRangeSelectionRenderObject extends _IMonthView {
  ...
  @override
  void updateSelection(PickerStateArgs details) {
    if (DatePickerHelper.isRangeEquals(details.selectedRange, selectedRange)) {
      return;
    }
...
class _MonthViewMultiRangeSelectionRenderObject extends _IMonthView {
  ...
  void updateSelection(PickerStateArgs details) {
    ...
    selectedRanges = DatePickerHelper.cloneList(details.selectedRanges);
    selectionNotifier.value = !selectionNotifier.value;
  }
}
...
int _getSelectedIndex(dynamic date, List<dynamic> visibleDates,
  ...
  for (int i = viewStartIndex; i < visibleDates.length; i++) {
    if (date.isSameDate(visibleDates[i])) {
      return i;
    }
  }
...
void _drawMonthCellsAndSelection(PaintingContext context, Size size,
  ...
  final bool isHorizontalMultiView = monthView.enableMultiView &&
      monthView.navigationDirection == DatePickerNavigationDirection.horizontal;
  final bool isVerticalMultiView = monthView.enableMultiView &&
      monthView.navigationDirection == DatePickerNavigationDirection.vertical;
  ...
  if (isNeedWidgetPaint) {
    ...
    for (int j = 0; j < viewCount; j++) {
      final int currentViewIndex =
          monthView.isRtl ? DatePickerHelper.getRtlIndex(viewCount, j) : j;
      ...
      for (int i = 0; i < datesCount; i++) {
        ...
        final bool isEnableDate = DatePickerHelper.isEnabledDate(
            monthView.minDate,
            monthView.maxDate,
            monthView.enablePastDates,
            date,
            monthView.isHijri);
        final bool isBlackedDate = DatePickerHelper.isDateWithinVisibleDates(
  ...
  final dynamic today = DatePickerHelper.getToday(monthView.isHijri);
  for (int j = 0; j < viewCount; j++) {
    final int currentViewIndex =
        monthView.isRtl ? DatePickerHelper.getRtlIndex(viewCount, j) : j;
    final dynamic currentMonthDate = monthView
        .visibleDates[((j * datesCount) + (datesCount / 2)).truncate()];
    final int nextMonth = currentMonthDate.getNextMonthDate().month;
    final int previousMonth = currentMonthDate.getPreviousMonthDate().month;
    bool isCurrentDate;
    ...
    for (int i = 0; i < datesCount; i++) {
      ...
      isCurrentDate = date.isSameDate(today);
      final bool isEnableDate = DatePickerHelper.isEnabledDate(
          monthView.minDate,
          monthView.maxDate,
          monthView.enablePastDates,
      ...
      final bool isWeekEnd =
          DatePickerHelper.isWeekend(monthView.weekendDays, date);
      final bool isSpecialDate = DatePickerHelper.isDateWithinVisibleDates(
