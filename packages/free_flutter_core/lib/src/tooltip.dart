...
class FfTooltip extends StatefulWidget {
  /// Creating an argument constructor of FfTooltip class.
  FfTooltip(
      ...
      this.textAlignment = FfTooltipAlignment.near,
  ...
  final void Function(FfTooltipRenderArgs tooltipRenderArgs)? onFfTooltipRender;
  ...
  void show(Offset? position, int duration, [Widget? template]) {
    if (position != null) {
      ...
      // ignore: avoid_as
      final FfTooltipState state = tooltipKey.currentState as FfTooltipState;
  ...
  @override
  FfTooltipState createState() => FfTooltipState();
}

/// Represents the state class of [FfTooltip] widget
class FfTooltipState extends State<FfTooltip>
...
class FfTooltipRenderObject extends SingleChildRenderObjectWidget {
  /// Creating an argument constructor of FfTooltipRenderObject class.
  FfTooltipRenderObject(
      {Widget? template,
      required FfTooltipState tooltipState,
  ...
  final FfTooltipState _tooltipState;
  final Animation<double> _tooltipAnimation;
  final AnimationController _animationController;

  @override
  FfTooltipRenderBox createRenderObject(BuildContext context) {
    _tooltipState.renderBox = FfTooltipRenderBox(
  ...
  void updateRenderObject(
      BuildContext context, FfTooltipRenderBox renderObject) {
    renderObject
...
class FfTooltipRenderBox extends RenderShiftedBox {
  /// Creating an argument constructor of FfTooltipRenderBox class.
  FfTooltipRenderBox(
      this._tooltipState, this._tooltipAnimation, this._animationController,
      [RenderBox? child])
      : super(child);
  FfTooltip get _tooltip => _tooltipState.widget;
  FfTooltipState _tooltipState;

  ///Setter for tooltipState
  set tooltipState(FfTooltipState value) {
  ...
  /// Getter for x coordinate of tooltip
  double? get x => _x;

  ///Getter for y coordinate of tooltip
  double? get y => _y;

  /// Getter for isLeft property of tooltip
  bool get isLeft => _isLeft;

  /// Getter for isRight property of tooltip
  bool get isRight => _isRight;

  /// Getter for isTop property of tooltip
  bool get isTop => _isTop;

  /// Getter for isOutOfBoundInTop property of tooltip
  bool get isOutOfBoundInTop => _isOutOfBoundInTop;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_tooltipState._show) {
      if ((_animationFactor == 0 || _tooltip.animationDuration == 0) &&
          _tooltipState.widget.onFfTooltipRender != null) {
        final FfTooltipRenderArgs tooltipRenderArgs = FfTooltipRenderArgs(
            _header,
            _stringValue,
            _x != null && _y != null ? Offset(_x!, _y!) : null);
        _tooltipState.widget.onFfTooltipRender!(tooltipRenderArgs);
      ...
      if (_tooltip.builder == null) {
        _renderDefaultFfTooltipView(context.canvas);
      } else {
        _renderTemplateFfTooltipView(context, offset);
      }
    }
    context.canvas.translate(-offset.dx, -offset.dy);
  }

  void _renderTemplateFfTooltipView(PaintingContext context, Offset offset) {
  ...
  void _renderDefaultFfTooltipView(Canvas canvas) {
    _isLeft = false;
    _isRight = false;
  ...
  void _drawFfTooltipBackground(
    ...
    if (!_canResetPath) {
      ...
      _drawFfTooltipPath(canvas, tooltipRect, rect, backgroundPath, isTop,
          isLeft, isRight, startX, endX, animationFactor, xPosition, yPosition);
      ...
      _drawFfTooltipText(
          canvas, tooltipRect, textStyle, result, animationFactor);
  ...
  void _drawFfTooltipText(Canvas canvas, RRect tooltipRect, TextStyle textStyle,
      Size result, double animationFactor) {
    const double padding = 10;
    final int _maxLinesOfFfTooltipContent = getMaxLinesContent(_stringValue);
    if (_header!.isNotEmpty) {
      ...
      if (_tooltip.format != null && _tooltip.format!.isNotEmpty) {
        if (_tooltip.textAlignment == FfTooltipAlignment.near) {
          lineOffset = padding;
        } else if (_tooltip.textAlignment == FfTooltipAlignment.far) {
          lineOffset = -padding;
        }
      }
